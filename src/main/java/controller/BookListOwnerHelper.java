...
public class BookListOwnerHelper {
	...
	public BookListOwner findOwner(String nameToLookUp) {
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<BookListOwner> typedQuery = em.createQuery("SELECT oh FROM BookListOwner oh WHERE oh.ownerName = :selectedName", BookListOwner.class);
		typedQuery.setParameter("selectedName", nameToLookUp);
		typedQuery.setMaxResults(1);
		BookListOwner foundOwner;
		try {
			foundOwner = typedQuery.getSingleResult();
		} catch (NoResultException ex) {
			foundOwner = new BookListOwner(nameToLookUp, nameToLookUp);
		}
		em.close();
		return foundOwner;
	}
	...
	public BookListOwner searchForOwnerById(int idToEdit) {
		EntityManager em = emfactory.createEntityManager();				//new instance of Entity Manager Factory
		em.getTransaction().begin();
