...
//		Date localDate = format.parse(date);
		...
		BookRatings br = new BookRatings(bookRating, localDate);
		

		//create BookWithRating Object with BookItem and BookRating
		BookWithRating bwr = new BookWithRating(bookToAdd, br);
...
//		}
		...
		listToEdit.getListOfItems().add(bwr);
...
////		bld.setListOfRatings(bookListItemRatings);
		bldh.insertBookListDetails(listToEdit);
//
 ...
 * value="${currentItem.id}" id="bookid">${currentItem.book}|
 * ${currentItem.author} | ${currentItem.genre}</option> </c:forEach> </select>
 * <!-- Make fields to enter date and ratings --> <label for="rating">Rate Book:
 * </label><input type="number" name="rating" placeholder="1-5"> <label
